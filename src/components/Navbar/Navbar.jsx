...
const Navbar = ({carts, setActiveTab}) => {
  ...
  return (
    <div className="drawer drawer-end lg:w-[90%] mx-auto">
      <input id="main-drawer" type="checkbox" className="drawer-toggle" />
      <div className="drawer-content flex flex-col">
        {/* Navbar Container */}
        <header className="navbar px-4 md:px-10 h-20">
          ...
          {/* Navbar right part (Cart, Login, Button, menu icon) */}
          <div className="flex-1 justify-end flex items-center gap-2 md:gap-4">

            {/* cart */}
            <a onClick={()=> setActiveTab("cart")} href="#cart-section">
              ...
              <div className="indicator">
                ...
                {/* Red Circle Badge */}
                {
                  carts.length < 1 ? "" : <span className="indicator-item badge badge-sm h-5 w-5 rounded-full border-none bg-[#FF4157] text-white text-[10px] font-bold flex items-center justify-center p-0">
                  {carts.length}
                </span>
                }
              </div>
            ...
            {/* btn */}
            <button className="hidden md:block btn bg-[#7C3AED] hover:bg-[#6D28D9] text-white border-none rounded-full lg:px-6 min-h-0 lg:h-11 capitalize shadow-md">
              Get Started
            </button>
            {/* menu icon */}
      ...
      <div className="drawer-side z-50 ">
        ...
        <ul className="menu p-6 w-64 min-h-full bg-base-100 text-lg font-medium ml-auto">
          <div className="mb-5 px-3 text-2xl font-bold text-[#7C3AED]">
            DigiTools
          </div>
          <li>
            <a onClick={closeDrawer} href="#products">Products</a>
          </li>
          <li>
            <a onClick={closeDrawer} href="#features">Features</a>
          </li>
          <li>
            <a onClick={closeDrawer} href="#pricing">Pricing</a>
          </li>
          <li>
            <a>Testimonials</a>
          </li>
